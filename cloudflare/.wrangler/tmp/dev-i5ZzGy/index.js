...
// .wrangler/tmp/bundle-t9puRH/checked-fetch.js
...
function getCorsHeaders(origin, allowedOrigin) {
  ...
  const isAllowed = allowedOrigins.includes(origin) || origin.endsWith(".pages.dev") || origin.endsWith(".workers.dev");
  return {
...
tombola.patch("/lots/:id/reserve", requireAuth, async (c) => {
  try {
    ...
    await c.env.DB.prepare(`
      UPDATE tombola_lots SET statut = 'r\xE9serv\xE9', reserved_by = ? WHERE id = ?
...
app.get("/init-db", async (c) => {
  try {
    const initSql = `
      ...
      -- Cr\xE9ation de la table user_roles
      ...
      -- Cr\xE9ation de la table sessions
      ...
      -- Cr\xE9ation de la table tombola_participants
      CREATE TABLE IF NOT EXISTS tombola_participants (
        ...
        classes TEXT,
        emoji TEXT DEFAULT '\u{1F60A}',
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (user_id) REFERENCES users(id)
      );

      -- Cr\xE9ation de la table tombola_lots
      CREATE TABLE IF NOT EXISTS tombola_lots (
        id TEXT PRIMARY KEY,
        nom TEXT NOT NULL,
        description TEXT,
        icone TEXT,
        statut TEXT DEFAULT 'disponible' CHECK (statut IN ('disponible', 'r\xE9serv\xE9', 'remis')),
      ...
      -- Cr\xE9ation de la table audit_logs
      ...
      -- Cr\xE9ation de la table rate_limits
...
// .wrangler/tmp/bundle-t9puRH/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-t9puRH/middleware-loader.entry.ts
