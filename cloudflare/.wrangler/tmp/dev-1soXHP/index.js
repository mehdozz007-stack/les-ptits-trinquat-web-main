...
// .wrangler/tmp/bundle-URiqE6/checked-fetch.js
...
auth.post("/login", authRateLimitMiddleware, async (c) => {
  console.log("[AUTH] POST /login - Requ\xEAte re\xE7ue", {
    method: c.req.method,
    url: c.req.url,
    headers: {
  ...
  try {
    const body = await c.req.json();
    console.log("[AUTH] Body re\xE7u:", { email: body.email, hasPassword: !!body.password });
    ...
    if (!user) {
      ...
      return c.json({
        success: false,
        error: "Email non trouv\xE9. Inscrivez vous !"
      }, 401);
    }
...
auth.post("/logout", requireAuth, async (c) => {
  try {
    ...
    return c.json({
      success: false,
      error: "Une erreur s'est produite lors de la d\xE9connexion"
    }, 500);
  }
});
auth.get("/me", requireAuth, async (c) => {
  ...
  if (!authContext) {
    return c.json({
      success: false,
      error: "Vous n'\xEAtes pas authentifi\xE9"
    }, 401);
  }
  return c.json({
    success: true,
...
auth.post("/change-password", requireAuth, async (c) => {
  try {
    const authContext = getAuthContext(c);
    if (!authContext) {
      return c.json({ success: false, error: "Vous n'\xEAtes pas authentifi\xE9" }, 401);
    }
    const body = await c.req.json();
    ...
    return c.json({
      success: true,
      message: "Votre mot de passe a \xE9t\xE9 chang\xE9 avec succ\xE8s"
    });
  } catch (error) {
...
function renderNewsletterEmail(props) {
  const {
...
<body style="background-color: #f5f5f5; padding: 20px 0;">
  <div class="email-container">
    ...
    <div class="header">
      ...
      <div class="header-subtitle">Association des Parents d'\xC9l\xE8ves</div>
    </div>
    
    <!-- Body -->
    <div class="body">
      <p class="greeting">Bonjour ${escapeHtml2(firstName)},</p>
      
      <h1 class="newsletter-title">${escapeHtml2(title)}</h1>
      ...
      <p style="margin-top: 30px; font-style: italic; color: #999999; font-size: 14px;">
        \u2014 L'\xE9quipe des P'tits Trinquat
      </p>
    </div>
    ...
    <div class="footer">
      <p class="footer-text">
        <strong>Les P'tits Trinquat</strong> \u2014 Association des Parents d'\xC9l\xE8ves
      </p>
      
      <p class="footer-text">
        <a href="${siteUrl}">Visiter notre site</a> \u2022 
        <a href="${siteUrl}/actualites">Actualit\xE9s</a>
      </p>
      
      <p class="footer-text" style="color: #999999;">
        \xA9 ${year} Les P'tits Trinquat. Tous droits r\xE9serv\xE9s.
      </p>
      
      <div class="unsubscribe">
        <p>Vous recevez cet email car vous \xEAtes inscrit \xE0 la newsletter de l'association Les P'tits Trinquat.</p>
        <p><a href="${unsubscribeUrl}">Se d\xE9sinscrire de la newsletter</a></p>
      </div>
    </div>
  </div>
</body>
...
__name(renderNewsletterEmail, "renderNewsletterEmail");
...
__name(sanitizeToHtml, "sanitizeToHtml");
function generateUnsubscribeUrl(email, baseUrl = "https://lespetitstrinquat.fr") {
  const encoded = btoa(email).replace(/\+/g, "-").replace(/\//g, "_").replace(/=/g, "");
  return `${baseUrl}/api/newsletter/unsubscribe?token=${encoded}`;
}
__name(generateUnsubscribeUrl, "generateUnsubscribeUrl");
  ...
  try {
    ...
    for (const subscriber of subscribers.results) {
      try {
        ...
        const logoUrl = c.env.ENVIRONMENT === "development" ? "https://lespetitstrinquat.fr/logoAsso-og.jpg" : `${c.env.SITE_URL}/logoAsso-og.jpg`;
        ...
        console.log(`\u{1F4E7} Sending to ${subscriber.email}:`, {
        ...
        console.log(`\u{1F4EC} Response for ${subscriber.email}:`, {
        ...
        results.push({
          ...
          success: sendSuccess,
          error: sendSuccess ? void 0 : `HTTP ${response.status} - ${responseText}`
        });
        if (sendSuccess) {
          try {
            await c.env.DB.prepare(`
...
newsletter.post("/admin/test-email", requireAdmin, async (c) => {
  try {
    ...
    console.log("\u{1F4E7} Sending to Resend with:", {
    ...
    console.log("\u{1F4EC} Resend response:", {
      status: response.status,
...
newsletter.delete("/admin/newsletters/:id", requireAdmin, async (c) => {
  try {
    const { id } = c.req.param();
    const authContext = getAuthContext(c);
    const newsletter2 = await c.env.DB.prepare(
...
tombola.delete("/admin/participants/:id", requireAdmin, async (c) => {
  try {
    ...
    return c.json({
      success: true,
      message: "Participant et utilisateur supprim\xE9s compl\xE8tement. Les donn\xE9es peuvent \xEAtre r\xE9utilis\xE9es."
    });
  } catch (error) {
...
// .wrangler/tmp/bundle-URiqE6/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-URiqE6/middleware-loader.entry.ts
