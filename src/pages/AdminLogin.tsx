...
import { Mail, Lock, Loader2, AlertCircle, LayoutGrid } from "lucide-react";
...
export default function AdminLogin() {
    ...
    const { user, isLoading: isAuthLoading, signIn, isAuthenticated } = useUnifiedAdminAuth();
    ...
    }, [user, isAuthLoading, isAuthenticated, navigate]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        console.log('[AdminLogin] Tentative de connexion avec email:', email);

        try {
            const { error } = await signIn(email, password);

            if (error) {
                console.error('[AdminLogin] Erreur de connexion:', error);
            ...
            console.error('[AdminLogin] Exception:', msg);
            setDebugError(msg);
        } finally {
            setIsLoading(false);
        }
                ...
                >
                    ...
                    <div className="text-center space-y-3">
                        ...
                        <p className="text-sm sm:text-base text-muted-foreground">
                            Connectez-vous pour gérer tous les espaces
                        </p>
                    </div>
                    ...
                    >
                        ...
                        <div className="bg-orange-50/50 border border-orange-200 rounded-lg p-4 text-xs sm:text-sm text-gray-700 space-y-2">
                            ...
                            <p>
                                Cette page est réservée aux administrateurs. Une fois connecté, vous accéderez à tous les espaces d'administration.
                            </p>
                        </div>
                    </motion.div>
