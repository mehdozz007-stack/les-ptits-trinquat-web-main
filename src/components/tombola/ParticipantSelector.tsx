...
export function ParticipantSelector({ currentParticipant, onSelect }: ParticipantSelectorProps) {
  ...
  const { participants, fetchMyParticipants, loading } = useTombolaParticipants();
  const [isOpen, setIsOpen] = useState(false);

  // Charger les participants personnels quand on a un user_id
  useEffect(() => {
    if (userId) {
      fetchMyParticipants(userId);
    }
  }, [userId, fetchMyParticipants]);
